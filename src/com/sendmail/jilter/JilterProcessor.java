...
import com.sendmail.jilter.internal.*;
...
public class JilterProcessor
{
     ...
     * Public constructor.
     * 
     * @param handler the underlying handler that will receive calls based on the Milter conversation.
     */
    ...
    private boolean processCurrentPacket(WritableByteChannel writeChannel)
        throws IOException
    {
...
//            log.debug(">SMFIC command is '" + ((char) this.packet.getCommand()) + "', Raw packet data:" + Util.newline() + Util.hexDumpLong(this.packet.getData()));
    ...
    private void processOptnegPacket(WritableByteChannel writeChannel)
        throws IOException
    {
        ...
        log.debug(
            "Supported flags " +
            Integer.toHexString(supported) +
            " maps to SMFIP_ flags " +
    ...
    private void processBodyPacket(WritableByteChannel writeChannel)
        throws IOException
    {
        sendReplyPacket(writeChannel, this.handler.body(this.packet.getData()));
    }

    private void processEOHPacket(WritableByteChannel writeChannel)
        throws IOException
    {
        sendReplyPacket(writeChannel, this.handler.eoh());
    }

    private void processHeaderPacket(WritableByteChannel writeChannel)
        throws IOException
    {
        String name = null;
        String value = null;
    ...
    private void processBodyEOBPacket(WritableByteChannel writeChannel)
        throws IOException
    {
        JilterStatus        status;
    ...
    private void processRcptPacket(WritableByteChannel writeChannel)
        throws IOException
    {
        String argv[] = null;
        ByteBuffer dataBuffer = this.packet.getData();
    ...
    private void processMailPacket(WritableByteChannel writeChannel)
        throws IOException
    {
        String argv[] = null;
        ByteBuffer dataBuffer = this.packet.getData();
    ...
    private void processHeloPacket(WritableByteChannel writeChannel)
        throws IOException
    {
    ...
    private void processConnectPacket(WritableByteChannel writeChannel)
        throws IOException
    {
    ...
    private void sendReplyPacket(WritableByteChannel writeChannel, JilterStatus status)
        throws IOException
    {
